...
export const createOrderRouter = ({ orderModel }: { orderModel: OrderModel }) => {
  ...
  router.post('/', authRequired, validateSchema(createOrderSchema, 'body'), controller.create)

  router.get('/:id', authRequired, validateSchema(orderIdSchema, 'params'), controller.getById)
